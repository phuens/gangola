...
export const TimberByFinishProductOrderList = ({
  userState,
  commonState,
  navigation,
  setLoading,
        ...
        <ListItem
          ...
          <Body>
            {item.docstatus === 0 ? (
              ...
              <Text numberOfLines={3}>{item.item_name}</Text>
            )}
  ...
  const getAllOrders = async () => {
    try {
      const response = await callAxios(
        `resource/Customer Order?fields=["name","docstatus", "site", "item_name", "creation", "total_balance_amount", "total_payable_amount"]&filters=[["user", "=", ${
          userState.login_id
        }],["product_category", "=","` +
  ...
  return commonState.isLoading ? (
    ...
    <Container style={globalStyles.listContent}>
      {showAlert && (
        <View style={{width: '100%', height: '100%'}}>
          <AwesomeAlert
            ...
            onConfirmPressed={() => {
              toggleAlert();
              cancelOrder(orderNo).then(function() {
                getAllOrders();
              });
            }}
          />
      ...
      <Button
        ...
        onPress={() => navigation.navigate('TimberByFinishProductOrder')}>
